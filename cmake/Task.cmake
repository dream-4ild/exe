...
set(LIBS_LIST "twist;sure;vvv;fmt;function2;nlohmann_json::nlohmann_json")
...
set(TESTING_LIBS_LIST "wheels;course-testing")
...
function(add_task_executable BINARY_NAME)
    ...
    target_link_libraries(${BINARY_NAME} PUBLIC pthread ${LIBS_LIST})
    add_dependencies(${BINARY_NAME} ${LIBS_LIST})
endfunction()

function(link_task_testing_libs TARGET_NAME)
    target_link_libraries(${TARGET_NAME} PUBLIC ${TESTING_LIBS_LIST})
    add_dependencies(${TARGET_NAME} ${TESTING_LIBS_LIST})
endfunction()
...
macro(set_task_sources)
    prepend(TASK_SOURCES "${TASK_SOURCE_DIR}/" ${ARGV})
endmacro()
...
function(add_task_library DIR_NAME)
    ...
    set(LIB_DIR ${TASK_SOURCE_DIR}/${DIR_NAME})
    ...
    if (LIB_CXX_SOURCES)
        ...
        project_log("Include dir: ${LIB_INCLUDE_DIR}")
        target_link_libraries(${LIB_TARGET} PUBLIC ${LIBS_LIST})
        add_dependencies(${LIB_TARGET} ${LIBS_LIST})
    else()
        # header-only library
...
function(add_task_test BINARY_NAME)
    ...
    link_task_testing_libs(${TEST_NAME})
